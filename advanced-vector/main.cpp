                ...
                if (rhs.size_ > size_) {
                    for (size_t i = 0; i != rhs.size_; ++i) {
                        if (i != this->size_) {
                            ...
                            std::uninitialized_copy_n(rhs.data_.GetAddress() + i, size_, data_.GetAddress() + i);
                            break;
                        }
                    }
                } else {
        ...
        if (dist == size_) {
            ...
            if (size_ >= Capacity()) {
                ...
                if constexpr (std::is_nothrow_move_constructible_v<T> || !std::is_copy_constructible_v<T>) {
                    ...
                    std::uninitialized_move_n( data_.GetAddress() + dist + 1, size_ - dist, new_data.GetAddress() + dist + 1);
                } else {
                    std::uninitialized_copy_n(data_.GetAddress(), dist - 1, new_data.GetAddress());
                    std::uninitialized_copy_n(data_.GetAddress() + dist, size_ - dist + 1, new_data.GetAddress() + dist);
                }
                std::destroy_n(data_.GetAddress(), size_);
                data_.Swap(new_data);
            } else {
